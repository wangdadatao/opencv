...
for i in range(0, PosNum):
    fileName = "D:/Documents/python/cup/poss/" + str(i + 263) + ".jpg"
...
for i in range(0, NegNum):
    fileName = "D:/Documents/python/cup/neg/" + str(i + 1) + ".jpg"
    img = cv.imread(fileName)
    # 计算Hog特征
    hist = hog.compute(img, (8, 8))  # 3780维
    for j in range(0, featureNum):
...
imageSrc = cv.imread('D:/Documents/python/cup/114.jpg', 1)

# 用训练好的特征 对目标进行检测：
objs = myHog.detectMultiScale(imageSrc, 0, (8, 8), (32, 32), 1.01, 1)
x = int(objs[0][0][0])
y = int(objs[0][0][1])
w = int(objs[0][0][2])
h = int(objs[0][0][3])
